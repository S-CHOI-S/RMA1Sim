...
namespace raisim {
    ...
    public:
        ...
        explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
            ...
            goalPos_ << 1.0, 0.0, 0.0;
            goalVel_ << 0.0, 0.0, sqrt(2 * 10 * (finalBoxHeight_ + 0.4));
            ...
            gc_init_ << 0, 0, 0.33, 1.0, 0.0, 0.0, 0.0, 0.05, 0.8, -1.4, -0.05, 0.8, -1.4, 0.05, 0.8, -1.4, -0.05, 0.8, -1.4;
        ...
        float step(const Eigen::Ref<EigenVec>& action) final {
            ...
            for(int i=0; i< int(control_dt_ / simulation_dt_ + 1e-10); i++){
                ...
                float xyError = posError_.head(2).norm();
                
                double bodyDirectionError = (gc_.head(2) / gc_.head(2).norm() - bodyLinearVel_.head(2) / bodyLinearVel_.head(2).norm()).norm();
                double landingError = 0.2 * (gv_init_.tail(12) - gv_.tail(12)).norm() + (gc_init_.tail(12) - gc_.tail(12)).norm() + abs(current_box_height + 0.33 - gc_[2]);
                if(xyError < boxSize_ / 2){
                    jumpReward_ += jumpCoeff_ / (3 * landingError + 1);
                    positionReward_ += positionCoeff_ / (3 * xyError + 1);
                }
                else{
                    jumpReward_ += jumpCoeff_ / (2 * velError_ + 4 * bodyDirectionError + 1);
                    if(allFeetInAir){
                        positionReward_ += positionCoeff_ / (3 * xyError + 1);
                    }
                }
